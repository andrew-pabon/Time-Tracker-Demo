-- 00009_report_function.sql
--
-- get_monthly_report(): returns aggregated time entries for a customer+month,
...
as $$
    ...
    where te.customer_id = p_customer_id
      and te.entry_date >= p_month
      and te.entry_date < (p_month + interval '1 month')::date
    group by
        sl.name,
        sl.display_order,
        ws.name,
...
comment on function public.get_monthly_report(uuid, date, boolean) is
    'Returns aggregated time entries for a customer+month, grouped by service line and workstream. Optional weekly breakdown.';
